...
class StudentPastExamResultsViewModel : ViewModel(){
    ...
    private val _homeworks = MutableStateFlow<List<StudentDashboard>>(emptyList())
    val homeworks : StateFlow<List<StudentDashboard>> = _homeworks
    ...
    fun dashboard (studentId : Long){
        viewModelScope.launch{
            _errorMessage.value = null


            try{
                val studentHomeworks = repository.dashboard(studentId)

                if (studentHomeworks.success){
                    _homeworks.value = studentHomeworks.data

                }else{
                    _errorMessage.value = "Bir hata oluştu ${studentHomeworks.message}"
