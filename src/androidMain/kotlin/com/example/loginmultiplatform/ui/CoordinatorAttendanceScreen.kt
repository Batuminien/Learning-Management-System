...
@Composable
actual fun CoordinatorAttendanceScreen(studentViewModel: AttendanceViewModel, teacherViewModel: TeacherAttendanceViewModel, navController: NavController) {
    ...
    if(showStudentDetail != null && selectedCourseId != null) {
        ...
        StudentAttendanceDetailDialogCoordinator(
                            ...
                            modifier = Modifier
                                ...
                                .onFocusChanged { focusState ->
                                    if (focusState.isFocused) {
                                        if (!hasUserModified && (isPastDate || selectedDate == today)) {
                                            textFieldValue = ""
                                        }
                                    }
                                },
...
@Composable
fun StudentAttendanceDetailDialogCoordinator(
    onDismiss: () -> Unit,
...
fun CustomSnackbar(
    ...
    SnackbarHost(
        ...
        snackbar = { data ->
            val isSuccess = data.message.contains("başarılı", ignoreCase = true)
            ...
            Box(
                modifier = Modifier
                    ...
                    .height(25.dp)
                    .background(
                        color = if (isSuccess) Color(0xFFEDF2FE) else Color(0xFFFFC1C1),
                ...
                content = {
                    Row(
                        ...
                        Icon(
                            ...
                            contentDescription = null,
                            tint = if (isSuccess) Color(0xFF334BBE) else Color.Red,
                            modifier = Modifier.size(24.dp)
                        )
                        Spacer(modifier = Modifier.width(8.dp))
                        Text(
                            text = data.message,
                            color = if (isSuccess) Color(0xFF334BBE) else Color.Red,
                            fontSize = 12.sp,
                            fontFamily = customFontFamily,
                            fontWeight = FontWeight.Bold
                        )
...
fun generateReportForStudent(
    ...
    val teacherCourseId = selectedCourseId
    val allAttendanceForStudent = attendanceMapSingular[studentId] ?: emptyList()

    val filteredAttendanceData = allAttendanceForStudent.filter {
        it.courseId.toInt() == teacherCourseId &&
                it.classId.toInt() == classInfo.id &&
                isDateInRange(it.date, startDate, endDate)
    }

    val allStatsForThisClass = attendanceStatsMap[studentId to classInfo.id] ?: emptyList()
    ...
    val studentCourseList = studentCoursesMap[studentId] ?: emptyList()
...
fun isDateInRange(date: String, startDate: String, endDate: String): Boolean {
    return try {
        ...
        parsedDate != null && parsedStartDate != null && parsedEndDate != null &&
                !parsedDate.before(parsedStartDate) && !parsedDate.after(parsedEndDate)
    } catch (e: Exception) {
        false
    }
}
